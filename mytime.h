...
inline int64_t GetTimeInUsec() {
  struct timeval now;
  gettimeofday(&now, NULL);
  return static_cast<int64_t>(now.tv_sec * 1000 * 1000 + now.tv_usec);
}
...
inline void timeval2timespec(timeval *const tv,
  ...
  int64_t now_nsec = static_cast<int64_t>(tv->tv_sec) * ten_9;
  now_nsec += static_cast<int64_t>(tv->tv_usec) * ten_3;
