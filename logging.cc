...
uint64_t TickCount() {
  ...
  uint64_t absolute_micro = static_cast<uint64_t>(ts.tv_nsec) / 1000;
...
std::ostream& operator<<(std::ostream &out, const LogAdditionInfo &info) {
  ...
  if (value) {
    out << "[bid:" << reinterpret_cast<uint64_t>(value) << "] ";
  }

  return out;
}

void LogAdditionInfo::AddBusinessIDByThread(uint64_t bid) {
